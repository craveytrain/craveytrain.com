...
- **h-entry microformats** on the post itself: `.h-entry`, `.p-name`, `.dt-published`, `.e-content`, `.p-category` (from `layouts/post.njk`, on every post). `.u-url` and `.h-card` appear separately, marking the _reply author_ in `includes/replies.njk`, rendered only when a post has `in-reply-to` webmentions.
